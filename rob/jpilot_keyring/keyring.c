 ...
 *
 * original code Copyright (C) 2001 by Judd Montgomery
 * zaurus keyring modifications Copyright (C) 2006 by Robert Miller
 *
...
static GtkWidget *import_file_button;  //rtm
...
static void cb_import_file(GtkWidget *widget, gpointer data);  //rtm
...
set_new_button_to(int new_state)
{
   ...
   switch (new_state) {
    ...
    case CLEAR_FLAG:
      ...
      gtk_widget_show(import_file_button);  //rtm
   ...
   switch (record_changed) {
    ...
    case CLEAR_FLAG:
      ...
      gtk_widget_hide(import_file_button);  //rtm
      //gtk_widget_hide(copy_record_button);
      gtk_widget_hide(delete_record_button);
      break;
   }
...
// rtm hacked this out
static int dialog_input_generic(GtkWindow *main_window, char *rslt, char *txtlabel, int visibility)  
{
   ...
   */
   ...
   label = gtk_label_new(_(txtlabel));
   ...
   strncpy(rslt, Pdata->text, PASSWD_LEN);
...
static int dialog_password(GtkWindow *main_window, char *ascii_password, int reason)
{
   ...
   if (reason==PASSWD_ENTER_RETRY) {
      return dialog_input_generic(main_window,ascii_password,"Incorrect, Reenter KeyRing Password",FALSE);
   } else if (reason==PASSWD_ENTER_NEW) {
      return dialog_input_generic(main_window,ascii_password,"Eenter a NEW KeyRing Password",FALSE);
   } else {
      return dialog_input_generic(main_window,ascii_password,"Enter KeyRing Password",FALSE);
   }
...
int plugin_gui(GtkWidget *vbox, GtkWidget *hbox, unsigned int unique_id)
{
   ...
   //rtm
   import_file_button = gtk_button_new_with_label(_("Import File"));
   gtk_box_pack_start(GTK_BOX(temp_hbox), import_file_button, TRUE, TRUE, 0);
   gtk_signal_connect(GTK_OBJECT(import_file_button), "clicked",
		      GTK_SIGNAL_FUNC(cb_import_file),
		      GINT_TO_POINTER(CLEAR_FLAG));
...
static void add_new_record_from_file(char *name, char *acct, char *pwd, char *note, char *category) {
   ...
   //br.attrib = glob_category_number_from_menu_item[glob_detail_category];
   br.attrib = glob_category_number_from_menu_item[0];
   jp_logf(JP_LOG_DEBUG, "category is %d\n", br.attrib);
   br.buf = buf;
   br.size = new_size;
   br.unique_id = 0;
...
static int parse_txt (char *str, char *start_delim, char *txt, char *fin_delim) {
  int fdlen;
  int tlen;
  char *ndx=&(str[0]);
  
  fdlen = strlen(fin_delim);

  while (ndx[0] == '\t' || ndx[0] == ' ' ) ndx++;

  //fprintf(stderr,"pt: sd %s  str >%s<  fd %s\n",start_delim,ndx,fin_delim);

  if (strncmp(ndx,start_delim,strlen(start_delim))) return FALSE;
  ndx += strlen(start_delim);
  if (*ndx != '\n')
    strcpy(txt,ndx);
  tlen = strlen(txt);
  if (fdlen && tlen > fdlen) {
    //fprintf(stderr,"ptm: >%s<\n",&(txt[tlen-fdlen-1]));
    if (!strncmp(fin_delim,&(txt[tlen-fdlen-1]),strlen(fin_delim))) {
      txt[tlen-fdlen-1] = '\0';
    }
  }

  //fprintf(stderr,"ptf: %s\n",txt);
  return TRUE;
}


static int find_text (char *str, char *targ) {
  char word[64];
  if (sscanf(str,"%s",word)) {
    if (!strcmp(word,targ)) 
      return TRUE;
  }
  return FALSE;
...
static void cb_import_file(GtkWidget *widget, gpointer data)  //rtm
{
   ...
   w = GTK_WINDOW(gtk_widget_get_toplevel(widget));
   r = dialog_input_generic(w, filename, "input filename",TRUE);
   
   if (r!= 1) return;

   if ((pwdfile = fopen(filename,"r")) == (FILE*) NULL) {
     char errmsg[2*FILENAME_LEN];
     sprintf(errmsg,"cannot open %s for reading",filename);
     dialog_generic_ok(widget,"error","file error",errmsg);
     return;
   }

   while (fgets(scanline,SCANLINE_LEN,pwdfile)) {

     //fprintf(stderr,"%s",scanline);

     if (find_text(scanline,"<account>")) {
       sprintf(name," ");
       sprintf(category," ");
       sprintf(user," ");
       sprintf(password," ");
       ...
       note_insert = 0;
       in_note=FALSE;
     } else if (find_text(scanline,"</account>")) {
       //fprintf (stderr,"finished n: %s c: %s u: %s p: %s n: %s\n",name,category,user,password,note);

       add_new_record_from_file(name,user,password,note,category);

     } else if (find_text(scanline,"<note></note>")) {
     } else if (parse_txt(scanline,"<note>",note,"</note>")) {
       in_note=TRUE;
       note_insert = strlen(note);
     } else if (find_text(scanline,"</note>")) {
       in_note=FALSE;
     } else if (parse_txt(scanline,"<name>",name,"</name>")) {
     } else if (parse_txt(scanline,"<category>",category,"</category>")) {
     } else if (parse_txt(scanline,"<user>",user,"</user>")) {
     } else if (parse_txt(scanline,"<password>",password,"</password>")) {
     } else if (in_note) {
       parse_txt(scanline,"",&(note[note_insert]),"</note>");
       note_insert = strlen(note);
     } else {
